...
import styles from "./Chat_temporary.module.css";
...
const Chat_temporary = () => {
  ...
  const [showSaveToPinecone, setShowSaveToPinecone] = useState(true);
...
const getAnswer = async () => {
  ...
  if (searchValue) {
    setIsOpen(false);
    try {
      const response = await fetch("http://192.168.0.182:8081/query-chroma", {
        method: "POST",
        headers: {
      ...
      while (true) {
        ...
        setMessages((prevMessages) => {
          ...
          lastMessage.answer = (lastMessage.answer || "") + chunk;
          return newMessages;
        });

        setAnswers((prevAnswer) => prevAnswer + chunk);
      }
  ...
  //       setMessages((prevMessages) => {
  //         const newMessages = [...prevMessages];
  //         newMessages[newMessages.length - 1].answer = data.response;
  //         return newMessages;
  //       });

  //       setCoheremsg((prevMessages) => {
  //         const newMessages = [...prevMessages];
  //         newMessages[newMessages.length - 1].answer = data.response;
  //         return newMessages;
  //       });
...
const saveToPinecone = async () => {
  try {
    const response = await axios.post(
      "http://192.168.0.182:8081/save-to-pinecone",
      {
        text: searchValue,
      }
    );
    console.log("Saved to Pinecone:", response.data);
