...
const Home = () => {
  ...
  const get_file_mail = async (fileName) => {
    const fileResponse = await axios.post(
      "http://localhost:5000/one_file_mail",
      { file: fileName },
  ...
  //       if (event.data.size > 0 && socket4) {
  //         chunks.push(event.data);
  //         socket4.emit('audio_chunk', event.data);
  //         console.log("emit", event.data.size)
  //       }
  //     };
  ...
  //         recorder.onstop = () => {
  //           if (chunks.length > 0 && socket4 && socket4.connected) {
  //               const blob = new Blob(chunks, { type: 'audio/webm; codecs=opus' });
  //               console.log('Sending recorded audio:', blob);
  //               socket4.emit('audio_chunk', blob);
  //               chunks.length = 0; // Clear chunks array for next recording
  //           }
  ...
  //             console.log('Sending recorded audio:', blob);
  //             socket4.emit('audio_chunk', blob);
  //             chunks.length = 0; // Clear chunks array after sending
  //         }
  //     }, 5000);

  //         recorder.start(); // This will trigger ondataavailable every 5 seconds
  //         setIsRecording(true);
  //     } catch (err) {
  //         console.error('Error accessing microphone:', err);
  //     }
  ...
  //       const sendAudioChunk = (audioChunks) => {
  //           if (audioChunks.length > 0 && socket4) {
  //               audioBuffer = audioBuffer.concat(audioChunks);
  //               const blob = new Blob(audioBuffer, { type: 'audio/wav; codecs=opus' });
  //               // transcribe(blob)
  //               console.log('Sending recorded audio:', blob);
  //               socket4.emit('audio_chunk', blob);
  ...
  //               sendAudioChunk(chunks);
  //               console.log("recording sent state")
      ...
      recognizer.recognized = (s, e) => {
        if (e.result.reason === ResultReason.RecognizedSpeech) {
          ...
          setTranscription((prevTranscription) => {
            ...
            console.log("RECOGNIZED:", correctedText);
            console.log("New Transcription:", newTranscription);
            setSearchValue(newTranscription);
            return newTranscription;
          });
